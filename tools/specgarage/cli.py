...
from . import profile
from .config import find_root, load_specs

PLANNED = {
    "build-vault": "Phase 1: sources/ → vault/ (tách note, gán ID, frontmatter, wikilink, copy ảnh)",
    ...
    "export": "Phase 1: vault/ → build/export/<CODE>.md theo _manifest.yaml",
}


def cmd_profile(args: argparse.Namespace) -> int:
    paths = args.paths or [find_root() / "sources"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("paths", nargs="*", type=Path, help="file .md hoặc thư mục (mặc định: sources/)")
    ...
    p.add_argument("--out", type=Path, help="ghi ra file, ví dụ reports/profile.txt")
