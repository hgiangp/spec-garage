...
|---|---|---|---|
| **§0** | Chặn rò rỉ dữ liệu, chuẩn bị môi trường | Chủ repo | — |
...
### 0.1 Repo đang public

Remote `github.com/hgiangp/spec-garage` hiện **public**. `.gitignore` đã có khối *PUBLIC-REPO GUARD* chặn `sources/`, `vault/<CODE>/`, `vault/attachments/`, `reports/`, `evals/`.

- [ ] **Quyết định:** chuyển repo sang **private** (khuyến nghị), hoặc chuyển dữ liệu sang git server nội bộ.
- Khi còn public:
  - Không gỡ guard, không `git add -f`.
  - Không commit nội dung domain vào `knowledge/`.
  - **Tag `baseline-original` (§4) sẽ không chứa vault** vì vault đang bị ignore. Xem 4.4.
- Khi đã private: xoá khối guard trong `.gitignore` trong một commit riêng, ghi rõ lý do.
...
### 0.3 Git
- Mỗi ticket là một branch `phase1/<ticket>-<mô-tả>` và một PR vào `main`.
- Commit dữ liệu spec (khi được phép) nằm ở branch `data/…` riêng, không trộn với code.
...
| Repo public, dữ liệu spec có thể bị commit nhầm | Rò rỉ spec | Thêm guard `.gitignore` và ghi rule vào `CLAUDE.md` |
...
### 2.1 Chạy
1. Chép 3 file spec và folder ảnh vào `sources/WRN/`, `sources/EWA/`, `sources/LIN/`, **giữ nguyên tên file**.
2. Điền `images:` trong `specs.yaml` (đường dẫn folder ảnh).
3. Chạy:
   ```bash
   uv run --project tools sg specs          # tất cả phải "ok"
   uv run --project tools sg profile --out reports/profile.txt
   uv run --project tools sg profile --json --out reports/profile.json
   ```
...
| `images missing files` > 0 | Sai đường dẫn ảnh | Sửa `images:` trong `specs.yaml` hoặc đường dẫn ảnh trong md |
...
**Manifest** `vault/<CODE>/_manifest.yaml`:
```yaml
spec: WRN
source: sources/WRN/7820ZXXXXG000_E_(Warning)_260220.md
source_sha256: …
...
- `allocate(code, n=1) -> list[str]`: lấy từ `next_id`, tăng và ghi lại. Format `f"{code}-{n:04d}"`; tự dùng 5 chữ số nếu `next_id > 9999`.
- `retire(id, merged_into=None, reason=…)`.
- Iterator trên cây manifest (thứ tự, cha/con).
- **Test:** cấp liên tiếp, không cấp lại ID đã retire, chuyển sang 5 chữ số.
...
6. Copy ảnh vào `vault/attachments/<CODE>/` và viết lại đường dẫn.
7. Ghi `_manifest.yaml` và sinh `_toc.md` (danh sách lồng nhau `[[ID|legacy title]]`).
8. **An toàn:**
   - Từ chối ghi đè `vault/<CODE>/` đã tồn tại nếu không có `--force`.
   - Nếu tag `baseline-original` đã tồn tại thì từ chối kể cả khi có `--force`, trừ khi có thêm `--i-know-baseline-exists`.
...
Wrapper CLI của `ids.allocate`, cần cho các skill khi tạo heading mới. **Nhớ thêm vào `PLANNED` / parser của `cli.py`** (hiện chưa có).
...
1. Build và kiểm tra:
   ...
   ```
2. **Expert mở `vault/` trong Obsidian** và kiểm tra khoảng 10 note ngẫu nhiên mỗi spec:
...
4. **Gắn baseline (bản "before"):**
   - **Repo đã private / nội bộ:** commit vault ở branch `data/baseline`, merge, rồi `git tag baseline-original`.
   - **Repo còn public:** vault bị ignore nên tag không chứa dữ liệu. Tạm thời lưu snapshot local, ví dụ:
     ```bash
     tar czf ~/spec-baselines/baseline-original-$(date +%Y%m%d).tgz vault sources specs.yaml
     ```
     Hoặc tạo một git repo local riêng cho `vault/`. **Không để phase improve bắt đầu khi chưa có baseline.**
...
2. **Chạy `spec-analyze`** trên từng section. Expert đọc report trong `reports/analyze/` và đánh dấu issue đúng/sai. Issue sai là dữ liệu để sửa skill hoặc checklist.
3. **Chạy `spec-restructure` và `spec-parameterize`** trên 1–2 section. Mỗi section một branch `improve/<ID>-…` và một PR.
4. **Expert review PR**, mỗi comment accept / reject / sửa **kèm lý do**. Sau khi merge:
   - Chưng cất lý do thành rule `L-00x` trong `knowledge/lessons.md`.
   - Thuật ngữ mới đưa vào `glossary.md`.
   - (Chỉ commit nội dung này khi repo đã private.)
5. **Tạo eval case:** mỗi section đã được duyệt thành một case trong `evals/skills/<skill>/` (input, expected, notes). Đủ 10–20 case thì chạy lại mỗi khi sửa skill.
...
|---|---|---|---|
| 1 | Chuyển repo sang private hay tách dữ liệu sang git nội bộ? | Chủ repo | §4 |
...
- Không gỡ PUBLIC-REPO GUARD trong .gitignore, không git add -f.
